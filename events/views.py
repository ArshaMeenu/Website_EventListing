...
class Dashboard(ListView):
            ...
            def get_context_data(self, **kwargs):
                ...
                cate_qs = event.values('categories').distinct()                      
                for i in cate_qs:
                    name = i['categories']
...
class UserProfileView(TemplateView):
           ...
           def get_context_data(self, *args, **kwargs):  
                      ...
                      for i in userevnt_obj:
                          ...
                          event_list.append({
                              'title':title,'description':desc,'location':location,'category':category
                          })                                       
                      context= {
...
def filter(request):   
    date =datetime.datetime.today().date()            
    qs = Event.objects.filter(end_date__gte=date,published = 'True')
    ...
    if is_valid_queryparam(sdate_search):
        qs = qs.filter(start_date__icontains=sdate_search)
    ...
    if is_valid_queryparam(edate_search):
        qs = qs.filter(end_date__icontains=edate_search)
    if query:
